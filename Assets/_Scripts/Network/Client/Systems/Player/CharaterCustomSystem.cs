...
namespace Community.Client.Systems
{
    public class CharaterCustomSystem : ComponentClient
    {
        ...
        {
            Entities.ForEach((ref RPlayerID playerID, ref CustomCharacter custom, ref CharacterCustomHead head, ref CharacterCustomBody body) =>
            {
                ...
                models.mesh_mask.materials[0].SetColor("_Color_body", head.Color_beard);

                if (custom.time < 100)
                {
                    models.mesh_mask.SetBlendShapeWeight(0, custom.time);
                    models.mesh_hat.SetBlendShapeWeight(0, custom.time);
                    models.mesh_mask.SetBlendShapeWeight(1, 0);
                    models.mesh_hat.SetBlendShapeWeight(1, 0);
                }
                else
                {
                    models.mesh_mask.SetBlendShapeWeight(0, 100);
                    models.mesh_hat.SetBlendShapeWeight(0, 100);
                    models.mesh_mask.SetBlendShapeWeight(1, custom.time - 100);
                    models.mesh_hat.SetBlendShapeWeight(1, custom.time - 100);
                }
                models.mesh_head.SetBlendShapeWeight(0, head.beardSize);
                models.mesh_mask.SetBlendShapeWeight(3, head.beardSize);
                models.mesh_mask.SetBlendShapeWeight(2, custom.massa);
