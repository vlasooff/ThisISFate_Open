    ...
    public class SplineEditor : Editor {
        ...
        void OnSceneGUI() {
            ...
            if (e.type == EventType.MouseDown) {
                ...
                if (e.capsLock) {
                    mustCreateNewNode = true; Debug.Log("mustCreateNewNode");
                }
            }
        ...
        public override void OnInspectorGUI() {
            ...
            if (GUILayout.Button("rest selected position"))
            {
                Undo.RegisterCompleteObjectUndo(spline, "delete spline node");
                selection.Position = Vector3.zero;
            }
            if (GUILayout.Button("rest selected rotation"))
            {
                Undo.RegisterCompleteObjectUndo(spline, "delete spline node");
                selection.Direction = Vector3.zero;
            }
            GUI.enabled = true;
